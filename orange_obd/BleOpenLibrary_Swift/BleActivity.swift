...
open class BleActivity: UIViewController,CBCentralManagerDelegate, CBPeripheralDelegate {
    ...
    open var connectblename:CBPeripheral?=nil
    ...
    open func centralManager(_ central: CBCentralManager, didDiscover peripheral: CBPeripheral, advertisementData: [String : Any], rssi RSSI: NSNumber) {
        ...
        if !(bles.contains(peripheral)){bles.append(peripheral)
        }
        if(peripheral != connectblename){return}
        central.stopScan()
        let user = UserDefaults.standard
    ...
    open func centralManager(_ central: CBCentralManager, didDisconnectPeripheral peripheral: CBPeripheral, error: Error?) {
        ...
        DispatchQueue.main.async {
            self.view.showToast(text: "Bluetooth is disconnected")
        }
        DispatchQueue.main.async {
            self.Disconnect()
        }
    ...
    open func GoMenu(){
        SetMenu(Pagememory[0])
    }
    open func SetMenu(_ newViewController: UIViewController){
            ...
            while(true){
                if(self.IsConnect||fal==10){
                    DispatchQueue.main.async {
                        ...
                        if(fal==10){self.connectblename=nil}
                    }
                    break
                }
                sleep(1)
